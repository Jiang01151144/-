...
def load_and_label(Classes, Labels,Numbers,data_by_class,collection):
    DATA_path='/mnt/data/房室肥大/DatainNPY/'
    for idx, class_data in enumerate(Classes):
        class_data = np.load(os.path.join('/mnt/data/房室肥大/DatainNPY/',Classes[idx]+'.npy'))
        data_label = Labels[idx]
        print(class_data.shape)
        print(data_label)
        if Classes[idx]==('RVH'):
            class_data=class_data[:178]
        if Classes[idx]==('RVEH'):
            class_data=class_data[:178]   
        if Classes[idx]==('RAH'):
        ...
        if Classes[idx]==('LVH'):
            class_data=class_data
        if Classes[idx]==('LVEH'):
            class_data=class_data[:294]
        if Classes[idx]==('LAH'):
            class_data=class_data[:178] 
        if Classes[idx]==('LAEH'):
            class_data=class_data[:178]
        if Classes[idx]==('Normal'):
            class_data=class_data[:356]            
        print(class_data.shape)
        for tmp_idx, tmp_data in enumerate(class_data):
            
            id_no = tmp_idx+1
            data_by_class[f'{Classes[idx]}'].append(collection(tmp_data,data_label,f'{Classes[idx]}/{id_no}'))
